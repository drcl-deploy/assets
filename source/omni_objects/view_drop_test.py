...
def _extract_object_xml(xml_path: Path, old_name: str, new_name: str,
                        x_offset: float, y_offset: float,
                        height: float) -> tuple[str, str]:
    ...
    assets_str = "\n".join(
        "    " + ET.tostring(c, encoding="unicode") for c in asset_el
    ) if asset_el is not None else ""

    bodies_str = "\n".join(
        "    " + ET.tostring(b, encoding="unicode") for b in wb.findall("body")
...
def run(obj_dirs: list[tuple[str, Path]]):
    ...
    print(f"  each pair: LEFT=hull | RIGHT=decomposed")
    print(f"  SPACE=pause  BACKSPACE=re-drop  (close window to exit)")
...
def main():
    ...
    ap.add_argument("paths", nargs="*", type=Path,
                    help="object dir(s) (e.g. ../omomo_objects/trashcan)")
    ap.add_argument("--all", action="store_true",
                    help="drop all objects from OBJECT_GROUP_PATTERNS at once")
    args = ap.parse_args()
