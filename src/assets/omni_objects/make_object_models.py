...
Generates (below the per-machine generated root):
...
Usage:
    assets generate
    assets generate --object smallbox
...
def _asset_abs(p: Path) -> str:
    ...
    resolved = p.resolve()
    if resolved.is_file() and resolved.stat().st_size < 256:
        header = resolved.read_bytes()[:128]
        if header.startswith(b"version https://git-lfs.github.com/spec/v1"):
            raise RuntimeError(f"Git LFS object is not materialized: {resolved}")
    return str(resolved)


def _metadata_asset_path(value: str) -> Path:
    """Resolve current and legacy package-relative metadata paths."""
    path = Path(value)
    if path.is_absolute():
        return path
    if path.parts and path.parts[0] in {"source", "assets"}:
        path = Path(*path.parts[1:])
    return ASSETS_DIR / path


def _output_dir(source_dir: Path, output_root: Path) -> Path:
    try:
        relative = source_dir.resolve().relative_to(ASSETS_DIR.resolve())
    except ValueError:
...
def compute_inertia_primitive(mass: float, collider: dict) -> tuple[list[float], dict]:
    ...
    if geom == "sphere":
        r = collider["radius"]
        moment = 2.0 / 5.0 * mass * r**2
        return com, dict(ixx=moment, iyy=moment, izz=moment, ixy=0.0, ixz=0.0, iyz=0.0)
    elif geom == "box":
...
def compute_inertia_mesh(obj_path: Path, mass: float) -> tuple[list[float], dict]:
    ...
    inertia = mesh.moment_inertia  # 3x3 about CoM
    return (
        [float(c) for c in mesh.center_mass],
        dict(
            ixx=inertia[0, 0],
            iyy=inertia[1, 1],
            izz=inertia[2, 2],
...
def _validate_inertia(name: str, inertia: dict) -> None:
    ...
    if w[0] <= 0:
        raise ValueError(
            f"{name}: inertia is not positive definite "
            f"(principal moments {np.round(w, 9).tolist()})"
        )
    if w[0] + w[1] < w[2] * (1 - 1e-9):
        raise ValueError(
            f"{name}: principal moments {np.round(w, 9).tolist()} violate the "
...
def resolve_inertial(name: str, metadata: dict, computed: tuple[list[float], dict]) -> dict:
    ...
    if over is None:
        ...
        if unknown:
            raise ValueError(
                f"{name}: unknown inertial.inertia keys {sorted(unknown)} "
    ...
    return dict(
        ...
        inertia=inertia,
        src=(
            "metadata" if "com" in block else "computed",
...
def _urdf_inertial(inr: dict) -> str:
    ...
    inertia = inr["inertia"]
    return (
        f'<mass value="{_fmt(inr["mass"])}"/>\n'
        f'      <origin xyz="{" ".join(_fmt(c) for c in inr["com"])}" rpy="0 0 0"/>\n'
        f"      <inertia "
        + " ".join(f'{k}="{_fmt(inertia[k])}"' for k in ("ixx", "ixy", "ixz", "iyy", "iyz", "izz"))
        + "/>"
    )
...
def _mjcf_inertial(inr: dict) -> str:
    ...
    inertia = inr["inertia"]
    return (
        f'<inertial pos="{" ".join(_fmt(c) for c in inr["com"])}" '
...
def _urdf_geom_tag(name: str, collider: dict | None, source_dir: Path) -> str:
    """Return the URDF <geometry> inner tag for mesh or primitive."""
    if collider is None:
        return f'<mesh filename="{_asset_abs(source_dir / f"{name}.obj")}"/>'
    geom = collider["geometry"]
    if geom == "sphere":
...
def write_urdf(
    ...
    inertial: dict,
    friction: float,
    collider: dict | None = None,
) -> Path:
    geom_tag = _urdf_geom_tag(name, collider, source_dir)
    out = output_dir / f"{name}.urdf"
    out.write_text(
        URDF_TEMPLATE.format(
            name=name, friction=friction, geom_tag=geom_tag, inertial=_urdf_inertial(inertial)
        )
    )
    return out
...
def write_cvx_hull_xml(
    ...
    xml = CVX_HULL_TEMPLATE.format(
        ...
        quat=metadata["initial_quat"],
    )
    out = output_dir / f"{name}_cvx_hull.xml"
    out.write_text(xml)
    return out
...
def write_primitive_xmls(
    ...
    for variant in ("cvx_hull", "cvx_dcmp"):
        ...
        out = output_dir / f"{name}_{variant}.xml"
        out.write_text(xml)
        paths.append(out)
...
def decompose_mesh(
    obj_path: Path, threshold: float, max_hulls: int, preprocess_resolution: int, seed: int
...
def write_cvx_dcmp_xml(
    source_dir: Path,
    output_dir: Path,
    name: str,
    metadata: dict,
    inertial: dict,
    parts: list[trimesh.Trimesh] | None,
    friction: float,
    subdir: str = "convex_decomp_meshes",
) -> Path:
    ...
    for i, part_path in enumerate(part_paths):
        ...
        part_assets.append(f'    <mesh name="{mname}" file="{_asset_abs(part_path)}"/>')
        part_geoms.append(
    ...
    out = output_dir / f"{name}_cvx_dcmp.xml"
    out.write_text(xml)
    return out
...
def process(
    ...
    *,
    ...
    name = obj_dir.name
    output_dir = _output_dir(obj_dir, output_root)
    ...
    if collider is not None:
        ...
        inertial = resolve_inertial(name, metadata, compute_inertia_primitive(mass, collider))
        _print_inertial(inertial, collider["geometry"])

        urdf_path = write_urdf(obj_dir, output_dir, name, inertial, friction, collider)
        print(f"  [urdf] {urdf_path.name}")

        hull_path, dcmp_path = write_primitive_xmls(
            output_dir, name, metadata, inertial, collider, friction
        )
    ...
    urdf_path = write_urdf(obj_dir, output_dir, name, inertial, friction)
    print(f"  [urdf] {urdf_path.name}")

    hull_path = write_cvx_hull_xml(obj_dir, output_dir, name, metadata, inertial, friction)
    print(f"  [hull] {hull_path.name}")
    ...
    if do_decompose:
        ...
        if existing_parts and not force:
            out = write_cvx_dcmp_xml(obj_dir, output_dir, name, metadata, inertial, None, friction)
            ...
            parts = decompose_mesh(obj_path, threshold, max_hulls, preprocess_resolution, seed)
            out = write_cvx_dcmp_xml(obj_dir, output_dir, name, metadata, inertial, parts, friction)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("paths", nargs="*", type=Path, help="object dir(s) or .obj file(s)")
    ap.add_argument(
        "--all", action="store_true", help="process every object in OBJECT_GROUP_PATTERNS"
    )
    ap.add_argument("--object", default=None, help="process a single object by name (with --all)")
    ap.add_argument("--force", action="store_true", help="regenerate even if outputs exist")
    ap.add_argument(
        "--no-decompose", action="store_true", help="skip CoACD decomposition (only URDF + hull)"
    )
    ap.add_argument(
        "--threshold", type=float, default=0.05, help="CoACD concavity threshold (0.02..0.1)"
    )
    ap.add_argument(
        "--max-hulls", type=int, default=32, help="cap on convex parts (default 32, -1 = no cap)"
    )
    ap.add_argument(
    ...
    ap.add_argument("--quiet", action="store_true", help="silence CoACD logging")
    output = ap.add_mutually_exclusive_group()
    output.add_argument(
        "--output-root", type=Path, help="generated asset root (default: user data directory)"
    )
    output.add_argument(
        "--in-place", action="store_true", help="write generated files beside tracked source assets"
    )
    args = ap.parse_args(argv)

    output_root = ASSETS_DIR if args.in_place else (args.output_root or GENERATED_ROOT)
    output_root = output_root.expanduser().resolve()
    print(f"Generated root: {output_root}")
    ...
    if args.all:
        targets = [(name, obj_dir) for name, obj_dir in discover_objects(args.object)]
        if not targets:
