...
DEFAULT_OUT_DIR = GENERATED_ROOT / "omni_objects" / "scenes"

# ── Object discovery ─────────────────────────────────────────────────────────


def discover_objects(
    filter_name: str | None = None, model: str = "hull", generated_root: Path = GENERATED_ROOT
    ...
    for pattern in OBJECT_GROUP_PATTERNS:
        if pattern.endswith(".urdf"):
            ...
            name = obj_dir.name
            generated_dir = generated_root / obj_dir.relative_to(ASSETS_DIR)
            xml_path = generated_dir / f"{name}{suffix}"
            if name not in seen and xml_path.is_file():
                seen.add(name)
                objects.append((name, xml_path, generated_dir))
            continue
        ...
        for match in sorted(ASSETS_DIR.glob(pattern)):
            if not match.is_dir() or match.name.startswith("__"):
                continue
            name = match.name
            generated_dir = generated_root / match.relative_to(ASSETS_DIR)
            xml_path = generated_dir / f"{name}{suffix}"
            if name not in seen and xml_path.is_file():
                seen.add(name)
                objects.append((name, xml_path, generated_dir))
...
def generate_scene(
    robot_xml_abs: str, object_name: str, object_xml_abs: str, alt_xml_abs: str | None
) -> str:
...
def main():
    ...
    parser.add_argument("--robot-xml", default=None, help="Absolute path to robot MuJoCo XML")
    ...
    parser.add_argument(
        "--out-dir", default=None, help=f"Output directory (default: {DEFAULT_OUT_DIR})"
    )
    parser.add_argument(
        "--generated-root",
        type=Path,
        default=GENERATED_ROOT,
        help="root containing generated object XML files",
    )
    ...
    objects = discover_objects(args.object, args.model, args.generated_root)
    if not objects:
        print(f"No objects found{f' matching {args.object!r}' if args.object else ''}.")
        print("  Did you run `assets generate` first?")
        return
    ...
    for name, obj_xml, obj_dir in objects:
        ...
        scene_xml = generate_scene(robot_xml_abs, name, str(obj_xml.resolve()), alt_abs)
